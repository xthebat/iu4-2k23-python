...
def check_length(text: str, length: int, l_index: int) -> bool:
    if l_index == -1:
        list_ = text.split(" ")
        ...
        while sorted_list:
            if sorted_list.stratswith("    -"):
                return len(sorted_list) < length


def divide_strings(text: str, length: int, l_index: int) -> list[str]:
...
def main(args: list[str]) -> int:
    ...
    if len(args) < 3:
        ...
        if n_index != -1 and args[n_index+1].isdigit:
            if check_length(text, int(args[n_index+1]), l_index):
            ...
            return -1
        substrings = divide_strings(text, length, l_index)
