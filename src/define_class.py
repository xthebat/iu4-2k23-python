...
class Define(BaseParsedObject):
    ...
    def print_object(self, filename: str) -> list:
        define_list = self.find_object(self, filename)
        define_object_list = []

        for element_define in define_list:
            define_name = self.__get_define_name(self, element_define)
            define_value = self.__get_define_value(self, element_define)
